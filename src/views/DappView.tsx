...
import {convertUtf8ToHex} from '../utils/encoding';
import {
  ERC20_ADDRESS_OBJECT,
  ERC721_ADDRESS_OBJECT,
  FIXTURE_NONCE,
  LINK_OBJECT_TOKEN_URI,
} from '../utils/fixture';

import ABI_ERC721_GOERLI from '../abi/erc721_goerli.json';
...
export const DAPPView = ({sdk}: DAPPViewProps) => {
  ...
  const signNonce = async () => {
    ...
    const params = [convertUtf8ToHex(FIXTURE_NONCE), from];
  ...
  const addERC20ToMetaMask = async () => {
    try {
      ...
      await ethereum?.request({
        ...
        params: {
          type: 'ERC20',
          options: {
            address: ERC20_ADDRESS_OBJECT,
            symbol: 'AUTH',
            decimals: 8,
            image:
              'https://authencity.infura-ipfs.io/ipfs/QmaR6wfdHvZS4Np7CgYyD7RGuZHeXt3vpNEWrdtDevUFtm',
          },
        },
      });
    } catch (e) {
  ...
  const mintERC721Token = async () => {
    const address = ERC721_ADDRESS_OBJECT;
    const abi = ABI_ERC721_GOERLI;
    // const selectedChainId = 5;
    const selectedChainId = '0x5';

    const signer = provider?.getSigner();

    const contract = new ethers.Contract(address, abi, signer);

    // stoped working after @metamask/sdk update from 0.1.0 to >= 0.2.0
    ...
    if (chainId !== selectedChainId) {
      throw new Error('The selected network does not match the currency');
    }

    // Random publication id
    const tokenId = Math.floor(Math.random() * 1000000000);

    const tokenPayload = `Publication: ${tokenId}; Nickname: @bilbo; Text: TestTestTest;`;
    const tokenURI = `${LINK_OBJECT_TOKEN_URI}/${tokenId}`;
    const tokenType = 'publication';

    await ethereum?.request({method: 'eth_requestAccounts'});

    const userWallets = (await ethereum?.request({
      method: 'eth_accounts',
    })) as string[];
    ...
    const mintResult = await contract.mintWithTokenObjectData(
    ...
    <View style={{borderWidth: 2, padding: 5}}>
      ...
      {connected ? (
        <>
          ...
          <Button title="Sign nonce" onPress={signNonce} />
          <Button title="Mint ERC721 Token" onPress={mintERC721Token} />
          <Button title="Watch asset" onPress={addERC20ToMetaMask} />
